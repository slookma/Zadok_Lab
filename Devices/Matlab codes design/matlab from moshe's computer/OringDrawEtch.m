function [] = OringDrawEtch(width,x,y,R,coupler, ring_width,contour,dir)
...
if strcmp(dir,'u')
   ...
   % busDraw(width,[x+0.5*coupler y+contour coupler],'l')
    next = QRindDrawEtch(width,x+0.5*coupler,y+contour,R,'RU');
    ...
    QRindDrawEtch(width,next5(1),next5(2),R,'DR')
end
if strcmp(dir,'d')
    %ring = cleRing(x,y-R-contour,width,R);
   %  busDraw(width,[x+0.5*coupler y-contour coupler],'l')
