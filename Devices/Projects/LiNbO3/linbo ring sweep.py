...
#periods = list(np.asarray(np.arange(-0.03, 0.03, 0.005)) + 0.967)
gaps = list(np.arange(0.8, 1.25, 0.05))
period =[0.96]
#fill_frac = 0.5
fill_factors =[0.4]# [0.4, 0.5,0.6]
GC_len = 60
taper_len = 30
...
for j in range(len(fill_factors)):  # j will take values 0 and 1
    ...
    for idx, gap in enumerate(gaps):
        ...
        path1.segment(0.2 * WG_length, **LAYER_WG)
        path2=gdspy.Path(width=WG_WIDTH, initial_point=(path1.x, path1.y+WG_WIDTH+gap))
        path2.turn(radius=300,angle=2*np.pi, layer=101, datatype=0)

        path1.segment(0.7 * WG_length, **LAYER_WG)
        cell.add(path1).add(path2)
        ...
        if negative:
            ...
            path_trench = gdspy.Path(WG_WIDTH + trench * 2, (center[0] - 3.23, center[1]))
            path_trench.segment(0.1*(WG_length ), '+x', **LAYER_NEG)
            sbend(path_trench, 500, 100, LAYER_NEG)
            path_trench.segment(0.2 * WG_length, **LAYER_NEG)
            
            
            path2_trench=gdspy.Path(width=WG_WIDTH + trench * 2, initial_point=(path_trench.x, path_trench.y+WG_WIDTH +gap))
            path2_trench.turn(300,2*np.pi,layer=301, datatype=0)
            
            path_trench.segment(0.7 * (WG_length ), '+x', **LAYER_NEG)
            
            # # Trench for Path2
            # trench_width = 3 * 2.1
            # trench_outer_radius = 150 + trench_width
            # trench = gdspy.Round((path1.x, path1.y + 2.1 + gap), trench_outer_radius,
            #                      inner_radius=150 - trench_width, initial_angle=0, final_angle=2*np.pi,
            #                      **{"layer": 49, "datatype": 0})
            # cell.add(trench).add(path2_trench)
            ...
            cell.add(arc).add(path_trench).add(arc2).add(path2_trench)

        # Add text for navigation (to track fill factors and periods)
        if idx % 2 ==0:
            text_content = f"{1000*gap:.0f}"
            ...
            cell.add(text)
            text_content1 = f"{idx}"
            text_position1 = (center[0] - 250, center[1])  # Adjust the position for text
        ...
        #     text = gdspy.Text(text_content, 50, text_position).rotate(np.pi/2)  # Added layer to text
        #     cell.add(text)
...
# Write to GDS file
lib.write_gds('ring_sweep_v3.gds')
