...
%     plotColor = [(i-1)/(length(widths)-1), 0, 1-(i-1)/(length(widths)-1)];
    if flag == 16
        plot(Lpeak.',kappa,'Color','b','Marker','+','MarkerSize',10,'DisplayName',['CP - \DeltaW = ' num2str(widths(i)) ' nm' ])
    else
        plot(Lpeak.',kappa,'Color','r','Marker','x','MarkerSize',10,'DisplayName',['Ref - \DeltaW = ' num2str(widths(i)) ' nm' ])
    end
    legend
    ylim([0 1])
    xlim([1538 1562])
