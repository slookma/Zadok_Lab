...
%     subplot(3,5,i)
    ...
    plotColor = [(i-1)/(length(widths)-1), 0, 1-(i-1)/(length(widths)-1)];
    if flag == 16
        plot(Lpeak_q.',viq,'LineWidth',0.5,'Color',plotColor,'DisplayName',['CP - \DeltaW = ' num2str(widths(i)) ' nm' ])%kappa,'b'
%         legend({['CP - \DeltaW = ' num2str(widths(i)) ' nm' ]});
    else
        plot(Lpeak_q.',viq,'LineWidth',0.5,'Color',plotColor,'DisplayName',['Ref - \DeltaW = ' num2str(widths(i)) ' nm' ],'LineStyle','--')%kappa,'r'
%         legend({['Ref - \DeltaW = ' num2str(widths(i)) ' nm' ]});
    end
    legend
    ylim([0 1])
    xlim([1538 1562])
